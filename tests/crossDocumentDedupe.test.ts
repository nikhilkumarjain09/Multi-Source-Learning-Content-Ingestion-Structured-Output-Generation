...
async function runCrossDocumentDedupeTests() {
  ...
  saveConcepts([conceptDoc1]);
  linkConceptToDocument(conceptDoc1.id, doc1Id);
  ...
  linkConceptToDocument(existingConcept.id, doc2Id);
  ...
  updateConceptDescription(existingConcept.id, richerDescription);
  ...
  // 3. Assertions
  const matchedConcepts = getConceptsByName(conceptName);
  if (matchedConcepts.length !== 1) {
    throw new Error(`Test Failed: Expected 1 concept row, got ${matchedConcepts.length}`);
  }

  const linkedDocs = getDocumentIdsForConcept(existingConcept.id);
  ...
  const artifacts = getArtifactsByTopic(conceptName);
  if (!artifacts) {
